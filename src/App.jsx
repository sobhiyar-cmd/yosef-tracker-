...
export default function App() {
  ...
  return (
    <div style={{
      background: "#111", minHeight: "100vh", color: "#ccc",
      fontFamily: "monospace", padding: "20px 16px", fontSize: 13,
      ...
      <div style={{ display: "flex", gap: 7, alignItems: "center", marginBottom: 16, flexWrap: "wrap" }}>
        ...
        {!gameOver && (
          <Btn
            ...
            gold={ready}
            glow={ready}
            style={{ animation: ready ? "glow 1.8s infinite" : "none" }}
          >
            Next →
          </Btn>
        )}
        ...
        {gameOver && (
          <Btn onClick={reset} style={{ borderColor: GOLD, color: GOLD }}>New Game</Btn>
        )}
        ...
        {!gameOver && (
          <Btn onClick={reset} style={{ borderColor: "#3a1a1a", color: "#7a3a3a" }}>Reset</Btn>
        )}
        ...
        <Btn
          ...
          style={{
            marginLeft: "auto",
            borderColor: showHistory ? GOLD : "#222",
            color: showHistory ? GOLD : "#555",
          }}
        >
      ...
      {showHistory && (
        <div style={{
          ...
          padding: 14, marginBottom: 18, animation: "fadeIn 0.2s ease",
        }}>
          ...
          {history.length === 0 ? (
            <div style={{ color: "#333", fontSize: 10, textAlign: "center", padding: "16px 0", letterSpacing: 2 }}>
              No saved games yet
            </div>
          ) : history.map(g => {
            const isOpen = expandedGame === g.id;
            return (
              <div key={g.id} style={{ borderBottom: "1px solid #171717", marginBottom: 1 }}>
                {/* Summary row */}
                <div
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                      <span style={{
                        ...
                        color: g.incomplete ? "#666" : GOLD,
                        textTransform: "uppercase", letterSpacing: 1,
                      }}>
                        {g.winner}
                      </span>
                      {g.incomplete && (
                        <span style={{ fontSize: 7, letterSpacing: 1, color: "#444", border: "1px solid #222", padding: "1px 4px" }}>PARTIAL</span>
                      )}
                      {g.gameName && (
                        <span style={{ fontSize: 8, color: "#555", fontStyle: "italic" }}>"{g.gameName}"</span>
                      )}
                    </div>
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); removeHistory(g.id); }}
                    style={{ background: "none", border: "none", color: "#2a2a2a", fontSize: 15, padding: "0 4px", flexShrink: 0 }}
                  >
                    ×
                  </button>
                </div>
                  ...
                  <div style={{ paddingBottom: 10, animation: "fadeIn 0.15s ease" }}>
                    {g.players.map((p, i) => (
                      <div key={i} style={{
                        ...
                        <span style={{
                          fontWeight: 700,
                          color: !p.out && p.name === g.winner ? GOLD : p.out ? "#2a2a2a" : "#666",
                        }}>
                          {p.score}
                        </span>
                        {!p.out && p.name === g.winner && (
                          <span style={{ fontSize: 9, color: GOLD, letterSpacing: 1 }}>★</span>
                        )}
                      </div>
                    ))}
                  </div>
      ...
      {!gameOver && (
        <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
          ...
          />
          <Btn onClick={add} gold style={{ padding: "6px 12px" }}>+ Add</Btn>
        </div>
      )}
      ...
      {players.map((p, i) => {
        ...
        return (
          <div key={p.id} style={{
            ...
            {p.out ? (
              <span style={{ fontSize: 8, letterSpacing: 2, textTransform: "uppercase", color: "#5a2222", border: "1px solid #3a1515", padding: "2px 5px" }}>
                Out
              </span>
            ) : !gameOver ? (
              <div style={{ display: "flex", gap: 5, alignItems: "center", flexShrink: 0 }}>
                <button
                  onClick={() => zero(p.id)}
                  style={{
                    background: isZ ? "#c9a22718" : "transparent",
                    border: `1px solid ${isZ ? GOLD : "#2a2a2a"}`,
                    color: isZ ? GOLD : "#555", fontFamily: "monospace", fontSize: 11, padding: "4px 8px",
                  }}
                >
                  0
                </button>
            ...
            <span style={{
              fontWeight: 900, fontSize: 20, minWidth: 36, textAlign: "right",
              color: p.out ? "#4a2222" : lead ? GOLD : "#bbb", flexShrink: 0,
            }}>
              {p.total}
            </span>
            ...
            {!gameOver && (
              <button
                onClick={() => del(p.id)}
                style={{ background: "none", border: "none", color: "#2a2a2a", fontSize: 16, padding: "2px 6px", flexShrink: 0 }}
              >
                ×
              </button>
            )}
      ...
      {/* Leaderboard + Win Prob panels */}
      {players.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: active.length > 0 && !gameOver ? "1fr 1fr" : "1fr", gap: 12, marginTop: 20 }}>
          {/* Leaderboard */}
          ...
          {/* Win probability (only while game is active) */}
